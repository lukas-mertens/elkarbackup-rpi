...
echo "DB: ${SYMFONY__DATABASE__HOST}"
echo "USER: ${SYMFONY__DATABASE__USER}"
echo "PW: ${SYMFONY__DATABASE__PASSWORD}"
# Check database connection
until mysqladmin ping -h "${SYMFONY__DATABASE__HOST}" -u "${SYMFONY__DATABASE__USER}" -p"${SYMFONY__DATABASE__PASSWORD}"; do
  >&2 echo "MySQL is unavailable - sleepiiiing"
  sleep 1
done
