# ElkarBackup
This is a fork of [elkarbackup/elkarbackup-docker](https://github.com/elkarbackup/elkarbackup-docker) build for elkarbackup to run on a raspberry pi.
To start using this image clone this repo, change the needed parameters in the `docker-compose.yml` file (e.g. the mysql-password and the backups directory) and run `sudo docker-compose up` in the main directory.
Docker-compose is going to create a container for the database and a container for elkarbackup. If you don't need the elkarbackup client, you can remove it from the `docker-compose.yml` file.
